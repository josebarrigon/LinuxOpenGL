...
int main()
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices1), vertices1, GL_STATIC_DRAW);//send data cpu -> array buffer -> vbo 
	...
	while (!glfwWindowShouldClose(window))
	{
		processInput(window);
		//Rendering commands
		glClearColor(1.0f, 0.3f, 0.3f, 1.0f); //state setting command
		glClear(GL_COLOR_BUFFER_BIT); //state using command 
		glUseProgram(shaderProgram);
		glBindVertexArray(VAO);
		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
		printSecondRectangle();
...
void printSecondRectangle() {
	...
	glBindBuffer(GL_ARRAY_BUFFER, VBO);//array buffer now points to vbo
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices2), vertices2, GL_STATIC_DRAW);//send data cpu -> array buffer -> vbo 
